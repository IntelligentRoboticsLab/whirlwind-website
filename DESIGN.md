...
The site self-hosts the same family: Google's variable `woff2` files for the latin and latin-ext subsets live in `public/fonts`, and `brand.css` declares them with `font-weight: 300 900` and `font-stretch: 75% 125%` (the optical size axis is driven by `font-optical-sizing: auto`), exposed as `--font-sans`. Fallback stack: `"TikTok Sans", "Helvetica Neue", Arial, sans-serif`. No request leaves the site for type.
...
The site is Astro, static output, no client-side framework. Everything below follows this guide; when the two disagree, fix the code.

- `src/styles/brand.css`: tokens, the TikTok Sans `@font-face` rules, type roles, links, the bib, the top bar, sponsor masks, the crest. `src/styles/globals.css`: page and component layout. No other stylesheet, no utility framework.
- `src/layouts/Base.astro`: the document: metadata, favicon and share image, the font preload, the top bar, header and footer. Every page passes it a title and description.
- `src/pages/*.astro`: one file per route; `src/pages/news/[slug].astro` builds a page per article. `404.astro` is the not-found page.
- `src/components/Name.astro` renders the name with the orange IRL; `src/lib/name.ts` does the same for strings (`withName`, rendered with `set:html`) and for rendered markdown. Never write the name as a bare string in a template.
- `src/components/Artwork.astro` picks the theme's version of a piece from `src/lib/artwork.ts`. Pieces are rendered by `tools/artwork`.
- `src/lib/images.ts`: the srcset widths every photograph and cutout is generated at, through `astro:assets`.
...
- `content/news/*.md`: articles, a content collection declared in `src/content.config.ts`, which validates the frontmatter. Frontmatter may carry `coverCredit` and `coverCaption`; the prose is never edited for design reasons. Cover and gallery photos are mapped in `src/lib/news` by id from `src/lib/photos.ts`.
...
- The scripts that run in the browser are the `<script>` in their component: the menu (`SiteHeader`), the photo dialog (`PhotoGrid`), the BibTeX copy (`BibtexButton`), the noise field (`NoiseField`) and the crest's secret (`FooterCrest`). Nothing else runs on the client.
- Old routes redirect in `astro.config.ts`: `/socials` to `/team`. `/sponsors` is a page again.
