...
Publications: title, one sentence saying what is here, then a list grouped by year with the year as `.t-heading`. Each entry: title in `.t-subheading` italic, authors in `.t-body`, type and date in `.t-meta`, then "PDF" and "BibTeX" as text links. Tags in `.t-meta` separated by commas, not pills. Software counts as a publication too, with "GitHub" in place of "PDF". A publication with a page of its own on the site (`project` in its entry) has its title as a link to that page and "Project page" as the first of its links.

A project page (`/publications/<slug>`, hand-written) is a research project page that once stood alone, brought into the site: the same sections, text and media in the same order, set in the site's type, colours, links and bib button. Nothing centred, nothing rounded, no shadows. It opens like every other page with the title and lede, then the actions in one row: the page's one bib button and the rest as links, with a brand's own logo, in its own colours at the height of a capital, beside a link where the original had one (the Hugging Face face). Later rows of actions are links only. Section headings in sentence case, a one-line subcaption in `.t-caption` under a heading where the original had one. The standalone page's affiliation logos, its "Hungry for more?" call and its template credits are not carried over: the site's header and footer already say who we are. One widget exists for these pages and nowhere else: the row of muted looping previews (`VideoCarousel`: the heading and two chevron arrows share one row above the track, the arrow keys browse too, only what is on screen plays, nothing under `prefers-reduced-motion`, a clip opens with sound in the same dialog as a photograph). Simulation renders arrive on a white ground; `.rendered` multiplies them onto the paper so the ground is the page, and on the dark canvas inverts and screens them for the same effect. Real video is never blended.
...
- A project page is hand-written in `src/app/publications/<slug>/page.tsx`, its images under `src/assets/publications/<slug>` and its videos under `public/publications/<slug>`; its entry in `src/lib/publications/publications.ts` points to it with `project`.
